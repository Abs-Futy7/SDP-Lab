...
class SynchronizedSingletonMongoDB:
    _instance = None
    _lock = threading.Lock()  # The lock that controls access

    @classmethod
    def get_instance(cls):
        with cls._lock:          # Only ONE thread can be inside here at a time
...
# Run all experiments
test_instance_validation(SynchronizedSingletonMongoDB, "Synchronized")

test_thread_safety(SynchronizedSingletonMongoDB, "Synchronized")

test_performance(SynchronizedSingletonMongoDB, "Synchronized")